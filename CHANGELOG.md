...
- `AGENTS.md` no longer ships inside the package tarball. It is generated by inlining the maintainer's `CLAUDE.md` and its `@`-imports, so it carried contributor workflow and references to unrelated internal projects — not consumer material. It was present in the tarball of 0.1.0–0.2.1. The file stays in the repo for cross-family reviewers.
